defmodule CorexAdmin.Live.Form.Controller do
  ...
  def handle_event("validate", params, socket) do
    with :ok <- authorize_form(socket) do
  ...
  defp load_relation_options(fields, scope) do
    ...
    |> Map.new(fn field ->
      options =
        if field.relation.search do
          []
        else
          Helpers.relation_options(field, scope, limit: field.relation.limit)
        end
  ...
  defp authorize_form(socket) do
    resource_mod = socket.assigns.resource_mod
    policy_action = if socket.assigns.live_action == :new, do: :new, else: :edit
    record = Map.get(socket.assigns, :record)
    Helpers.authorize(socket, policy_action, resource_mod, record)
  end
