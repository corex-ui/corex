defmodule CorexAdmin.UI.Fields do
  ...
  defp nested_input(assigns) do
    ...
    <div class="admin-nested">
      <fieldset class="nested-fields" data-scope="nested-fields">
        <legend class="admin-nested-legend">
          <span>{@field.label}</span>
          <.error_tip :if={@errors != []} id={@tip_id} msg={hd(@errors)} />
        </legend>
        <.inputs_for :let={nested} field={@form[@field.name]}>
          <div class="admin-nested-row">
            <.input :for={child <- @field.fields} field={child} form={nested} />
          </div>
        </.inputs_for>
        <button type="button" class="button ui-size-sm" data-part="add-trigger" disabled>
