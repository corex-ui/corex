defmodule Corex.Design.Theme.Options do
  ...
  defp prepare_theme_spec(theme_id, spec) when is_map(spec) do
  ...
  defp prepare_theme_spec(_theme_id, _spec) do
    {:error, "theme spec must be a map"}
  end
  ...
  defp reject_host_scale_keys_in_dimensions(spec, theme_id) do
    ...
    if is_map(dims) do
      ...
      if found do
        ...
        {:error,
         "themes.#{theme_id}: #{key_str} is not allowed in host config; " <>
           "use top-level scales: for spacing and text, or dimensions.radius step values"}
      else
        :ok
      end
    else
  ...
  defp validate_theme_spec(spec) when is_map(spec) do
    if resolved_spec?(spec) do
      with :ok <- validate_palette_hex(normalize_palette_for_hex(spec)) do
