...
المعدّلات كيف تضبط نسخة واحدة دون تفريع النظام كله. تعيش على class الجذر. الزر هو `button`. زر accent هو `button button--semantic-accent`. زر كبير مستدير هو `button button--semantic-accent button--size-lg button--rounded-lg`. نظام المعدّلات له أربعة محاور (لون، حجم، radius، نوع)، وتربط مباشرة بمتغيرات CSS تحت الغطاء.
...
إن فشلت تسمية في تدقيق، الجواب غالباً تبديل ثيم، أو وضع، أو معدّل (`button--semantic-accent` مقابل `button--semantic-muted`)، قبل أي override. الرموز هي المصدر. الـ overrides هي العرض.
