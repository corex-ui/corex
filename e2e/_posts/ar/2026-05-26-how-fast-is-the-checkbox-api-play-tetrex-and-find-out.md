...
Tetrex شبكة 10 في 18 من checkboxes Corex تتظاهر بأنها قطع Tetris. كل خلية `<.checkbox>` حقيقي، بـ id ثابت، ونفس آلة Zag تحت كل checkbox آخر في المكتبة، ومعدّلات BEM للون القطعة (`ui-accent` للقطعة accent، `ui-info` لـ info، وهكذا). عندما تسقط قطعة، تضيء الخلايا. عندما يُمسح صف، ينهار. عندما تنتهي اللعبة، يسجل النتيجة في لوحة متصدرين مدعومة بـ Phoenix Presence وGenServer لكل لعبة.
