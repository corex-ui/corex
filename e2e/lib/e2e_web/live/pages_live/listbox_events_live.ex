defmodule E2eWeb.ListboxEventsLive do
  use E2eWeb, :live_view

  import E2eWeb.DemoPage, only: [demo_page: 1, demo_section: 1]

  @id_server "listbox-events-server"
  @id_client "listbox-events-client"

  @server_heex E2eWeb.Demos.ListboxDemo.events_server_heex()
  @server_elixir E2eWeb.Demos.ListboxDemo.events_server_elixir()
  @client_heex E2eWeb.Demos.ListboxDemo.events_client_heex()
  @client_js E2eWeb.Demos.ListboxDemo.events_client_js()
  @client_ts E2eWeb.Demos.ListboxDemo.events_client_ts()
  ...
  def mount(_params, _session, socket) do
    socket =
      ...
      |> assign(:items, E2eWeb.Demos.ListboxDemo.items_minimal())
  ...
  def handle_event("listbox_client_changed", %{"id" => id, "value" => value}, socket) do
        ...
        >
          <:preview>
            <div class="flex flex-col gap-4 items-center w-full">
              ...
              <div
                id="listbox-events-client-listener"
                class="w-full"
                phx-hook=".ListboxEventsClient"
                phx-update="ignore"
              >
                <script :type={Phoenix.LiveView.ColocatedHook} name=".ListboxEventsClient">
                  export default {
                    mounted() {
                      const el = document.getElementById("listbox-events-client");
                      if (!el) return;
                      el.addEventListener("listbox-value-changed", (e) => {
                        const d = e.detail ?? {};
                        const value = Array.isArray(d.value) ? d.value : [];
                        this.pushEvent("listbox_client_changed", { id: d.id, value });
                      });
                    }
                  }
                </script>
