defmodule E2eWeb.PresenceApiLive do
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} mode={@mode} theme={@theme} path={@path}>
      <.demo_page
        path={@path}
        id="presence-api-page"
        title="Presence · API"
        subtitle="Toggle present from client bindings, client JS, or a server event. The trigger is outside the wrapper."
      >
          ...
          <:preview>
            <div class="flex flex-col gap-space items-center">
              <div class="flex gap-space-sm">
                <.action
                  phx-click={Corex.Presence.set_present("presence-api-cb", true)}
                  class="button ui-size-sm"
                >
                  Show
                </.action>
                <.action
                  phx-click={Corex.Presence.set_present("presence-api-cb", false)}
                  class="button ui-size-sm"
                >
                  Hide
                </.action>
          ...
          <:preview>
            <div class="flex flex-col gap-space items-center">
              <button
                type="button"
                class="button ui-size-sm"
                onclick="document.getElementById('presence-api-cjs')?.dispatchEvent(new CustomEvent('corex:presence:set-present', {bubbles: false, detail: { present: true }}))"
              >Show</button>
