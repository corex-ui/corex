defmodule E2eWeb.StylesheetTest do
  ...
  test "corex tailwind bundle exists after compile", %{conn: _conn} do
    ...
    assert base =~ "body h1, .typo h1"
    assert base =~ "@layer reset, base;"
    refute button =~ "@utility button--*"
    assert button =~ ".button.button--accent"
    assert button =~ ".button.button--solid.button--accent"
    ...
    assert accordion =~ ".accordion.accordion--accent"
  ...
  test "built site.css includes recipe styling via corex.tailwind.css", %{conn: _conn} do
    ...
    assert site =~ ".select"
    assert site =~ ".accordion--accent"
    assert site =~ "background-color:var(--color-accent)"
