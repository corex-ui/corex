...
`components:` lists the component recipes to emit. Omit the key or set `nil` for the full catalog. `semantics:` and `variants:` trim palette roles and surface variant utilities when you need a smaller bundle.
...
Then import the design layers from `assets/css/app.css`. The minimum is `main.css`, a theme, and the generated component entry:
...
`components.css` is generated by `mix corex.design.build` from the `components:` list in `config :corex_design`. Add or remove ids there instead of editing `app.css` per component.
