...
scaffolds, and wires the `:corex_design` compiler when design is enabled. LiveView, HTML, esbuild, and full Phoenix assets stay enabled  -  there are no `--no-live` / `--no-html` / `--no-esbuild` / `--no-assets` switches here.
