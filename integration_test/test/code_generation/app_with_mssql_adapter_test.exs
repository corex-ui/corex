  ...
  describe "corex.gen.auth + pbkdf2 + existing context" do
    test "has no compilation or formatter warnings (--live)" do
      with_installer_tmp("new with defaults", fn tmp_dir ->
        ...
        mix_run!(~w(corex.gen.html Accounts Group groups name), app_root_path)
        ...
        mix_run!(~w(corex.gen.auth Accounts User users --hashing-lib pbkdf2 --merge-with-existing-context --live), app_root_path)
    ...
    test "has no compilation or formatter warnings (--no-live)" do
      with_installer_tmp("new with defaults", fn tmp_dir ->
        ...
        mix_run!(~w(corex.gen.auth Accounts User users --hashing-lib pbkdf2 --merge-with-existing-context --no-live), app_root_path)
