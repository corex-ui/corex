defmodule Corex.Integration.CodeGeneration.CorexDesignOffTest do
  ...
  describe "--no-design" do
    test "with --replace produces class-free starter and Layouts.corex def" do
      with_installer_tmp("corex_design_off_replace", fn tmp_dir ->
    ...
    test "with --no-replace produces class-free starter and Layouts.corex def" do
      with_installer_tmp("corex_design_off_no_replace", fn tmp_dir ->
        {app_root_path, _} =
          generate_corex_app(tmp_dir, "my_app", ["--no-design", "--no-replace"])
