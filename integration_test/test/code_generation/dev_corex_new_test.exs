defmodule Corex.Integration.CodeGeneration.DevCorexNewTest do
  @moduledoc """
  End-to-end check for `mix corex.new ... --dev-corex <repo>` before publishing Hex.

  Uses `--no-design` so Igniter does not require `mix corex.design` during the install pipeline.
  """
  use Corex.Integration.CodeGeneratorCase, async: false

  if Mix.Task.get("phx.new") != nil do
    @tag timeout: 600_000
    test "mix corex.new with --dev-corex applies Corex install (esbuild ESM, hooks, config, web)" do
