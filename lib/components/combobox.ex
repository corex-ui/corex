defmodule Corex.Combobox do
  ...
  form_control_attrs(
    ...
    docs: [
      id: "The id of the combobox, useful for API to identify the combobox",
      field:
      ...
      form: "The id of the form of the combobox",
      read_only: "Whether the combobox is read only"
    ]
  )
  ...
  def combobox(%{field: %Phoenix.HTML.FormField{} = field} = assigns) do
    ...
    raw_value = field_value_list(field.value)
    value = normalize_value_to_ids(items, raw_value)
    selected_label = selected_label(items, value)
  ...
  defp prepare_combobox(assigns, field_used) do
    ...
    assigns =
      ...
      |> assign(:placeholder, translation.placeholder)
      |> assign(:empty_text, translation.empty)
      |> assign(:field_used, field_used)

    items = normalize_items(assigns.items)
    value_list = normalize_value_to_ids(items, field_value_list(Map.get(assigns, :value)))
    ...
    assigns =
      ...
      |> assign(:grouped_items, named_groups(items))
      |> assign(:value, value_list)
      |> assign(:selected_label, selected_label(items, value_list))
      |> assign(:value_for_hidden_input, value_for_hidden_input(value_list, assigns.multiple))
      |> assign_array_submit(field_used)

    assign(assigns, :hook_attrs, combobox_connect_props(assigns))
  end

  defp named_groups(items) do
    items
    |> group_by_group()
    |> Enum.reject(fn {group, _entries} -> is_nil(group) end)
  end

  defp assign_array_submit(%{multiple: true, name: name} = assigns, field_used)
       when is_binary(name) do
    assigns = Corex.FormField.assign_list_submit(assigns)
    assign(assigns, :empty_array_name, empty_array_name(field_used, assigns.submit_name))
  end

  defp assign_array_submit(assigns, _field_used) do
    assigns
  ...
  defp combobox_render(assigns, field_used) do
    assigns = prepare_combobox(assigns, field_used)
    ...
    {@hook_attrs}>
      ...
      />
        ...
        <div :if={!Enum.empty?(@label)} {Connect.mounted_label(%Label{id: @id, invalid: @invalid, read_only: @read_only, required: @required, disabled: @disabled, dir: @dir, orientation: @orientation})}>
          {render_slot(@label)}
        </div>
        <div {Connect.mounted_control(%Control{id: @id, invalid: @invalid, dir: @dir, disabled: @disabled, orientation: @orientation})}>
          <input value={@selected_label || ""} {Connect.mounted_input(%Input{id: @id, value: @value, selected_label: @selected_label, form: nil, invalid: @invalid, dir: @dir, disabled: @disabled, required: @required, placeholder: @placeholder, name: nil, auto_focus: @auto_focus, orientation: @orientation})} />
          <button :if={!Enum.empty?(@clear_trigger)} hidden={Enum.empty?(@value)} {Connect.mounted_clear_trigger(%ClearTrigger{id: @id, dir: @dir, disabled: @disabled, invalid: @invalid, orientation: @orientation})} aria-label={@translation.clear_selection}>
            {render_slot(@clear_trigger)}
          </button>
          <button {Connect.mounted_trigger(%Trigger{id: @id, dir: @dir, disabled: @disabled, invalid: @invalid, orientation: @orientation})} aria-label={@translation.trigger}>
            {render_slot(@trigger)}
          </button>
        </div>
        ...
        <div {Connect.mounted_positioner(%Positioner{id: @id, dir: @dir, orientation: @orientation})}>
          <div {Connect.mounted_content(%Content{id: @id, dir: @dir, orientation: @orientation})}>
            <ul {Connect.mounted_list(%List{id: @id, dir: @dir, orientation: @orientation})}>
              <li :if={@items == []} {Connect.mounted_empty(%Empty{id: @id, dir: @dir, orientation: @orientation})}>
                {if Enum.empty?(@empty), do: @empty_text, else: render_slot(@empty)}
              </li>

              <li :if={@has_groups} :for={{group_id, group_items} <- @grouped_items} {Connect.mounted_item_group(%ItemGroup{id: @id, group_id: group_id, dir: @dir, orientation: @orientation})}>
                <div {Connect.mounted_item_group_label(%ItemGroupLabel{id: @id, html_for: group_id, dir: @dir, orientation: @orientation})}>
                  {group_id}
                </div>
                <ul>
                  <li :for={entry <- group_items} phx-mounted={Connect.ignore_item(%Item{id: @id, item: entry, value: entry_value(entry), dir: @dir, orientation: @orientation})} {item_attrs(@id, entry, @dir, @orientation)}>
                    <span :if={Enum.empty?(@item)} {Connect.mounted_item_text(%ItemText{id: @id, item: entry, dir: @dir, orientation: @orientation})}>
                      {entry.label}
                    </span>
                    <span :if={!Enum.empty?(@item)} {Connect.mounted_item_text(%ItemText{id: @id, item: entry, dir: @dir, orientation: @orientation})}>
                      {render_slot(@item, entry)}
                    </span>
                    <span :if={!Enum.empty?(@item_indicator)} {Connect.mounted_item_indicator(%ItemIndicator{id: @id, item: entry, dir: @dir, orientation: @orientation})}>
      ...
      <Corex.Component.Errors.field_errors scope="combobox" errors={@errors} error={@error} />
      <div style="display: none;" data-templates="combobox">
        <li data-scope="combobox" data-part="empty" data-template="true">
          {if Enum.empty?(@empty), do: @empty_text, else: render_slot(@empty)}
        </li>

        <li :if={@has_groups} :for={{group_id, group_items} <- @grouped_items} {Connect.item_group_template(%ItemGroup{id: @id, group_id: group_id, dir: @dir, orientation: @orientation})} data-template="true">
          <div {Connect.item_group_label_template(%ItemGroupLabel{id: @id, html_for: group_id, dir: @dir, orientation: @orientation})}>
            {group_id}
          </div>
          <ul>
            <li :for={entry <- group_items} {item_attrs_template(@id, entry, @dir, @orientation)} data-template="true">
