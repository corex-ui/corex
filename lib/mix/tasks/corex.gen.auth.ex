defmodule Mix.Tasks.Corex.Gen.Auth do
  ...
  ```console
  $ mix corex.gen.auth Identity Client clients
  ```
  ...
  [Phoenix `mix phx.gen.auth` guide](https://hexdocs.pm/phoenix/mix_phx_gen_auth.html).

  > #### A note on scopes {: .info}
  >
  > `mix corex.gen.auth` creates a scope named after the schema by default.
  ...
  `mix corex.gen.auth` generates email based authentication, which assumes the user who
  ...
  workflows:
      ...
      $ mix corex.gen.auth Store User users
  ...
  a `--table` option may be provided. For example:
      ...
      $ mix corex.gen.auth Accounts User users --table accounts_users
  ...
  ```console
  $ mix corex.gen.auth Accounts User users --scope app_user
  ```
  ...
  ```console
  $ mix corex.gen.auth Accounts User users --assign-key current_user_scope
  ```

  This is useful when you want to run `mix corex.gen.auth` multiple times in the same project, but note that
  ...
  defp validate_required_dependencies! do
    unless Code.ensure_loaded?(Ecto.Adapters.SQL) do
      raise_with_help("mix corex.gen.auth requires ecto_sql", :phx_generator_args)
    end

    if generated_with_no_html?() do
      raise_with_help("mix corex.gen.auth requires phoenix_html", :phx_generator_args)
    end
  ...
  defp files_to_be_generated(binding) do
    ...
    default_files =
      [
        "migration.ex.eex": [
          migrations_pre,
          "#{timestamp()}_create_#{schema.table}_auth_tables.exs"
        ],
  ...
  defp inject_context_functions(%Context{file: file} = context, phoenix_paths, paths, binding) do
  ...
  defp inject_tests(%Context{test_file: test_file} = context, phoenix_paths, paths, binding) do
  ...
  defp maybe_inject_app_layout_menu(%Context{} = context, binding) do
    if file_path = get_layout_html_path(context) do
      ...
      Unable to find a layout file to inject user menu items.

      Missing files:

      #{missing}

      Please add the following code where you'd like the #{binding[:schema].singular} menu items to be rendered.
  ...
  defp potential_layout_file_paths(%Context{context_app: ctx_app}) do
    ...
    [
      ...
      Path.join([web_prefix, "components", "layouts", "app.html.heex"]),
      Path.join([web_prefix, "components", "layouts", "root.html.heex"])
    ]
  end
  ...
  defp raise_with_help(msg, :general) do
    ...
    mix corex.gen.auth expects a context module name, followed by
  ...
  defp raise_with_help(msg, :phx_generator_args) do
    ...
    mix corex.gen.auth must be installed into a Phoenix 1.5 app that
  ...
  defp raise_with_help(msg, :hashing_lib) do
    ...
    mix corex.gen.auth supports the following values for --hashing-lib
  ...
  defp put_live_option(context) do
    opts =
      case Keyword.fetch(context.opts, :live) do
        {:ok, false} ->
          reject_no_live!(live: false)

        {:ok, true} ->
          context.opts

        :error ->
          Keyword.put(context.opts, :live, true)
      end

    %{context | opts: opts}
  end

  defp reject_no_live!(opts) do
    if opts[:live] == false do
      Mix.raise("""
      mix corex.gen.auth is LiveView-only.

      Pass no `--live` flag (LiveView is the default), or use `mix phx.gen.auth --no-live`
      if you need controller-based authentication templates.
      """)
    end
  ...
  defp maybe_remap_for_test(context) do
    case mix_test_output() do
      nil ->
        context
      ...
      tmp ->
        schema = context.schema
        schema = %{schema | file: Path.join(tmp, "#{schema.singular}.ex")}

        %{
          context
          | file: Path.join(tmp, "#{context.basename}.ex"),
            test_file: Path.join(tmp, "#{context.basename}_test.exs"),
            test_fixtures_file: Path.join(tmp, "#{context.basename}_fixtures.ex"),
            dir: tmp,
            schema: schema
        }
    end
