defmodule Mix.Tasks.Corex.Gen.Context do
  ...
  ```console
  $ mix corex.gen.context Accounts User users name:string age:integer
  ```
  ...
  a `--table` option may be provided. For example:
      ...
      $ mix corex.gen.context Accounts User users --table cms_users
  ...
  Read the documentation for `corex.gen.schema` for more information on
  attributes.
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise(
        "mix corex.gen.context must be invoked from within your *_web application root directory"
      )
    end
  ...
  def ensure_context_file_exists(%Context{file: file} = context, paths, binding) do
    unless Context.pre_existing?(context) do
      Mix.Generator.create_file(
        file,
        Mix.Phoenix.eval_from(paths, "priv/templates/corex.gen.context/context.ex", binding)
      )
    end
  end
  ...
  defp inject_schema_access(%Context{file: file} = context, paths, binding) do
    ...
    |> Mix.Phoenix.eval_from(
      "priv/templates/corex.gen.context/#{schema_access_template(context)}",
      binding
    )
  ...
  def ensure_test_file_exists(%Context{test_file: test_file} = context, paths, binding) do
    unless Context.pre_existing_tests?(context) do
      Mix.Generator.create_file(
        test_file,
        Mix.Phoenix.eval_from(paths, "priv/templates/corex.gen.context/context_test.exs", binding)
      )
    end
  end
  ...
  defp inject_tests(%Context{test_file: test_file} = context, paths, binding) do
    ...
    paths
    |> Mix.Phoenix.eval_from("priv/templates/corex.gen.context/#{file}", binding)
  ...
  def ensure_test_fixtures_file_exists(
    ...
    unless Context.pre_existing_test_fixtures?(context) do
      Mix.Generator.create_file(
        test_fixtures_file,
        Mix.Phoenix.eval_from(paths, "priv/templates/corex.gen.context/fixtures_module.ex", binding)
      )
    end
  end
  ...
  defp inject_test_fixture(
    ...
    paths
    |> Mix.Phoenix.eval_from("priv/templates/corex.gen.context/fixtures.ex", binding)
  ...
  defp validate_args!(
    ...
    {context, schema, plural, schema_args} =
      if has_context? do
        ...
        # mix corex.gen.live User users name:string
        ...
        if context == maybe_context_name do
          # if someone did
          # mix corex.gen.live Users users name
          Mix.raise("""
          ...
          Please pass an explicit context option like:
              ...
              mix corex.gen.live #{context} #{maybe_context_name} #{schema_name_or_plural}
  ...
  def raise_with_help(msg) do
    ...
    mix corex.gen.html, corex.gen.json, corex.gen.live, and corex.gen.context
    ...
    For example:
        ...
        mix corex.gen.json [Accounts] User users name:string
        mix corex.gen.live [Accounts] User users name:string
        mix corex.gen.context Accounts User users name:string

    The context serves as the API boundary for the given resource.
    It is optional except for corex.gen.context.
