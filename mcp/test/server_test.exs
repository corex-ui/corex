defmodule Corex.MCP.ServerTest do
  ...
  setup do
    unless Application.get_env(:corex_mcp, :debug) || Application.get_env(:corex, :debug) do
      Logger.put_module_level(Corex.MCP.Server, :none)
    end

    Application.put_env(:corex_mcp, :mcp_test_raise_tool, true)

    on_exit(fn ->
      Application.delete_env(:corex_mcp, :mcp_test_raise_tool)
  ...
  test "handle_http_message notifications/initialized returns 202" do
    ...
    assert Corex.MCP.Json.decode!(conn.resp_body) == %{"status" => "ok"}
  end
  ...
  test "handle_http_message tools/call lists components" do
    ...
    payload = Corex.MCP.Json.decode!(inner["text"])
