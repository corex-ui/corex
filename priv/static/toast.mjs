...
// ../node_modules/.pnpm/@zag-js+toast@1.43.3/node_modules/@zag-js/toast/dist/toast.dom.mjs
...
// ../node_modules/.pnpm/@zag-js+toast@1.43.3/node_modules/@zag-js/toast/dist/toast.utils.mjs
var defaultTimeouts = {
  info: 5e3,
  error: 5e3,
  success: 2e3,
...
// ../node_modules/.pnpm/@zag-js+toast@1.43.3/node_modules/@zag-js/toast/dist/toast-group.connect.mjs
...
// ../node_modules/.pnpm/@zag-js+toast@1.43.3/node_modules/@zag-js/toast/dist/toast-group.machine.mjs
...
// ../node_modules/.pnpm/@zag-js+toast@1.43.3/node_modules/@zag-js/toast/dist/toast.connect.mjs
...
// ../node_modules/.pnpm/@zag-js+toast@1.43.3/node_modules/@zag-js/toast/dist/toast.machine.mjs
...
// ../node_modules/.pnpm/@zag-js+toast@1.43.3/node_modules/@zag-js/toast/dist/toast.store.mjs
...
// ../node_modules/.pnpm/@zag-js+toast@1.43.3/node_modules/@zag-js/toast/dist/index.mjs
var group = {
    ...
    for (const { type, body, title, duration, fallbackTitle } of readFlashToasts(this.el)) {
      try {
        store.create({
          ...
          duration: parseToastDuration(duration ?? void 0)
        });
      } catch (error) {
...
function parseToastDuration(duration) {
  if (duration === "Infinity" || duration === Infinity) return Infinity;
  if (typeof duration === "string") return parseInt(duration, 10) || void 0;
  return duration;
}
...
var ToastHook = createZagLiveHook({
  key: "toastGroup",
  mount(hook, { dom, server }) {
    ...
    const group2 = new ToastGroupHandle(el, {
      id: hook.groupId,
      placement,
    ...
    const matchesGroup = (payload) => typeof payload.group_id === "string" && payload.group_id === hook.groupId;
    ...
    const createToast = (payload, trusted) => {
      if (!matchesGroup(payload)) return;
      const st = getToastStore(payload.group_id);
      if (!st) return;
      try {
    ...
    const updateToast = (payload, trusted) => {
    ...
    server.add("toast_create", (payload) => createToast(payload, true));
    server.add("toast_update", (payload) => updateToast(payload, true));
    server.add("toast_dismiss", handleDismissPayload);
    server.add("toast_remove", handleRemovePayload);
    dom.add(
      "corex:toast:create",
      (event) => createToast(event.detail, false)
    );
    dom.add(
      "corex:toast:update",
      (event) => updateToast(event.detail, false)
    );
    dom.add(
      "corex:toast:dismiss",
      (event) => handleDismissPayload(event.detail)
    );
    dom.add(
      "corex:toast:remove",
      (event) => handleRemovePayload(event.detail)
    );
    return group2;
  }
});
