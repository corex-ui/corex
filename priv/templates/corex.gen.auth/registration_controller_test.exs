defmodule <%= inspect context.web_module %>.<%= inspect Module.concat(schema.web_namespace, schema.alias) %>RegistrationControllerTest do
  ...
  <% end %>describe "GET <%= schema.route_prefix %>/register" do
    test "renders registration page", %{conn: conn} do
      ...
      assert response =~ ~p"<%= if layout_locale do %>/#{@locale}<% end %><%= schema.route_prefix %>/log-in"
      assert response =~ ~p"<%= if layout_locale do %>/#{@locale}<% end %><%= schema.route_prefix %>/register"
    end

    test "redirects if already logged in", %{conn: conn} do
      conn = conn |> log_in_<%= schema.singular %>(<%= schema.singular %>_fixture()) |> get(~p"<%= if layout_locale do %>/#{@locale}<% end %><%= schema.route_prefix %>/register")
  ...
  describe "POST <%= schema.route_prefix %>/register" do
    ...
    test "creates account but does not log in", %{conn: conn} do
      ...
      conn =
        post(conn, ~p"<%= if layout_locale do %>/#{@locale}<% end %><%= schema.route_prefix %>/register", %{
