...
<% else %>
  ...
  def create(conn, %{<%= inspect schema.singular %> => <%= schema.singular %>_params}) do
    case <%= inspect context.alias %>.create_<%= schema.singular %>(<%= context_scope_prefix %><%= schema.singular %>_params) do
      {:ok, <%= schema.singular %>} ->
        ...
        |> redirect(to: ~p"<%= if layout_locale do %>/#{conn.params["locale"]}<% end %><%= scope_conn_route_prefix %><%= schema.route_prefix %>/#{<%= schema.singular %>}")
  ...
  def delete(conn, %{"<%= primary_key %>" => <%= primary_key %>}) do
    ...
    |> redirect(to: ~p"<%= if layout_locale do %>/#{conn.params["locale"]}<% end %><%= scope_conn_route_prefix %><%= schema.route_prefix %>")
  end
end
