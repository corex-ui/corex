...
<% end %>  }<%= if layout_locale_paths do %>
  ...
  describe "index" do
    test "lists all <%= schema.plural %>", %{conn: conn<%= test_context_scope %>} do
      conn = get(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>")
  ...
  describe "new <%= schema.singular %>" do
    test "renders form", %{conn: conn<%= test_context_scope %>} do
      conn = get(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/new")
  ...
  describe "create <%= schema.singular %>" do
    test "redirects to show when data is valid", %{conn: conn<%= test_context_scope %>} do
      conn = post(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>", <%= schema.singular %>: @create_attrs)

      assert %{<%= primary_key %>: <%= primary_key %>} = redirected_params(conn)
      assert redirected_to(conn) == ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/#{<%= primary_key %>}"

      conn = get(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/#{<%= primary_key %>}")
    ...
    test "renders errors when data is invalid", %{conn: conn<%= test_context_scope %>} do
      conn = post(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>", <%= schema.singular %>: @invalid_attrs)
      assert html_response(conn, 200) =~ "New <%= schema.human_singular %>"
    end
  end
  ...
  describe "edit <%= schema.singular %>" do
    ...
    test "renders form for editing chosen <%= schema.singular %>", %{conn: conn, <%= schema.singular %>: <%= schema.singular %><%= test_context_scope %>} do
      conn = get(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/#{<%= schema.singular %>}/edit")
  ...
  describe "update <%= schema.singular %>" do
    ...
    test "redirects when data is valid", %{conn: conn, <%= schema.singular %>: <%= schema.singular %><%= test_context_scope %>} do
      conn = put(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/#{<%= schema.singular %>}", <%= schema.singular %>: @update_attrs)
      assert redirected_to(conn) == ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/#{<%= schema.singular %>}"

      conn = get(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/#{<%= schema.singular %>}")<%= if schema.string_attr do %>
    ...
    test "renders errors when data is invalid", %{conn: conn, <%= schema.singular %>: <%= schema.singular %><%= test_context_scope %>} do
      conn = put(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/#{<%= schema.singular %>}", <%= schema.singular %>: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit <%= schema.human_singular %>"
    end
  end
  ...
  describe "delete <%= schema.singular %>" do
    ...
    test "deletes chosen <%= schema.singular %>", %{conn: conn, <%= schema.singular %>: <%= schema.singular %><%= test_context_scope %>} do
      conn = delete(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/#{<%= schema.singular %>}")
      assert redirected_to(conn) == ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>"

      assert_error_sent 404, fn ->
        get(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/#{<%= schema.singular %>}")
      end
    end
  end
