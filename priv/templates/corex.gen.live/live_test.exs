...
<% end %>
  ...
  describe "Index" do
    ...
    test "lists all <%= schema.plural %>", <%= if schema.string_attr do %>%{conn: conn, <%= schema.singular %>: <%= schema.singular %><%= test_context_scope %>}<% else %>%{conn: conn<%= test_context_scope %>}<% end %> do
      {:ok, _index_live, html} = live(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>")
    ...
    test "saves new <%= schema.singular %>", %{conn: conn<%= test_context_scope %>} do
      {:ok, index_live, _html} = live(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>")

      assert {:ok, form_live, _} =
               index_live
               |> element("a", "New <%= schema.human_singular %>")
               |> render_click()
               |> follow_redirect(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/new")
      ...
      assert {:ok, index_live, _html} =
               ...
               |> follow_redirect(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>")
    ...
    test "updates <%= schema.singular %> in listing", %{conn: conn, <%= schema.singular %>: <%= schema.singular %><%= test_context_scope %>} do
      ...
      assert {:ok, form_live, _html} =
               ...
               |> render_click()
               |> follow_redirect(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/#{<%= schema.singular %>}/edit")
  ...
  describe "Show" do
    ...
    test "displays <%= schema.singular %>", %{conn: conn, <%= schema.singular %>: <%= schema.singular %><%= test_context_scope %>} do
      {:ok, _show_live, html} = live(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/#{<%= schema.singular %>}")
    ...
    test "updates <%= schema.singular %> and returns to show", %{conn: conn, <%= schema.singular %>: <%= schema.singular %><%= test_context_scope %>} do
      {:ok, show_live, _html} = live(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/#{<%= schema.singular %>}")

      assert {:ok, form_live, _} =
               show_live
               |> element("a", "Edit")
               |> render_click()
               |> follow_redirect(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/#{<%= schema.singular %>}/edit?return_to=show")
      ...
      assert {:ok, show_live, _html} =
               form_live
               |> render_submit("save", %{"<%= schema.singular %>" => @update_attrs_params})
               |> follow_redirect(conn, ~p"<%= if layout_locale_paths do %>/#{@locale}<% end %><%= scope_param_route_prefix %><%= schema.route_prefix %>/#{<%= schema.singular %>}")
