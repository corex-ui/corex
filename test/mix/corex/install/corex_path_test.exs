defmodule Corex.Mix.Corex.Install.AssetsPathImportTest do
  ...
  test "path dep keyword-list form {:corex, [path: ...]} resolves to corex.mjs import" do
    root = Path.join(System.tmp_dir!(), "corex_path_kw_" <> id())
    app = Path.join(root, "my_app")
    src = Path.join(root, "corex_src")
    File.mkdir_p!(Path.join(app, "assets/js"))
    File.mkdir_p!(Path.join(src, "priv/static"))

    File.write!(Path.join(app, "mix.exs"), """
    defmodule MyApp.MixProject do
      def project, do: [app: :my_app, deps: [{:corex, [path: "../corex_src", override: true]}]]
    end
    """)
    ...
    try do
      File.cd!(app, fn ->
        assert [import_line: line] = Assets.js_patch_opts()
        assert line =~ "corex.mjs"
        refute line =~ ~s(import corex from "corex")
      end)
    after
      File.cd!(prev)
      File.rm_rf!(root)
